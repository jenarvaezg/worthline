...
Turborepo caches tasks under `.turbo/` locally. **Remote cache** (Vercel, free on Hobby) is enabled in CI when `TURBO_TOKEN` and `TURBO_TEAM` are set — see [`docs/agents/turbo-remote-cache.md`](docs/agents/turbo-remote-cache.md).
