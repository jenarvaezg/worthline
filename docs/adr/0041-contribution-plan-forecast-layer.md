...
- ~~**`monthlySavingsCapacity` becomes derived** — the sum of the active plan's monthly-equivalent contributions — with the manual scalar kept as the fallback when no plan exists. `projectFire` reads the derived value, so there are never two forward-savings inputs that disagree.~~ **Superseded by ADR 0074 (#1416).** The derivation substituted a subset (planned additions to named destinations) for a declared total, and it did so silently: Jorge declared 1.500 €/mes and the projection ran on the 100 €/mes of his single pension-plan row. The declared scalar is now the only savings input the FIRE projection takes; the plan keeps every other use listed here.
...
- ~~`monthlySavingsCapacity` is now derived — a new consumer of the forward-savings input; a **drift watch** with the FIRE reservation calc (#421 / #507) that already reuses it.~~ Retired by ADR 0074 (#1416): the drift watch this predicted is exactly the bug that landed.
