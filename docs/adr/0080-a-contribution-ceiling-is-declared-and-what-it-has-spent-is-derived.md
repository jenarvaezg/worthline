...
1. **A cupo is an entity that groups holdings.** `contribution_allowances` (schema v58)
   carries a label and one amount — `annual_cap_minor` — and
   `contribution_allowance_holdings` names the destinations that consume it. A set, not
   one holding: a tax ceiling belongs to the *contributor* and aggregates every plan he
   holds, so the second plan needs no migration. With one plan it behaves exactly like a
   per-holding cupo.
...
3. **Consumed = real buy operations of the calendar year, to the marked holdings.**
   Not occurrences, not `fulfilled` flags, not `storedExecutionMinor`: a contribution
   made without a plan row foreseeing it counts, because it happened. There is no stored
   total — `computeContributionAllowanceUsage` derives it on every read, so a corrected
   operation moves the counter with it and nothing can go stale behind the user's back.
...
6. **Only holdings with an operation ledger may be marked.** A stored-value destination
   records no entries one by one, so a cupo over it would count 0 — the counter lying
...
- /objetivos gains a "Cupo anual de aportación" panel next to the contribution plan:
  ...
  ceiling approaches and is passed, the audit fold, and the editor.
