  ...
  const tools: ToolSet = {
    ...
    propose_holding: tool({
      description:
        ...
        "family + instrument deben concordar: stored (current_account/term_deposit/precious_metal/vehicle/other) → currentValueMinor; appreciating (property) → currentValueMinor + isPrimaryResidence; debt (mortgage/loan/credit_card) → balanceMinor (+ debtModel si lo conoces); investment (fund/etf/stock/index/pension_plan/crypto) → isin/providerSymbol opcionales y, para valorar la apertura de hoy, openingValueMinor (importe efectivo en céntimos) + pricePerUnit; sin apertura crea un contenedor vacío. " +
        "Si el documento dice los TÍTULOS y la COMISIÓN (una confirmación de compra siempre los dice), pásalos en units y feesMinor (céntimos): se guardan tal cual y el coste sale exacto — sin units se derivan del importe y quedan falsas, con la comisión dentro. La app comprueba que units × pricePerUnit + feesMinor cuadre con openingValueMinor y avisa sin bloquear. " +
