...
- Sé conciso: conclusión primero, evidencia compacta después. No repitas la misma guía en otro párrafo ni cierres recapitulando lo ya dicho. Cita las cifras que uses (importe y fecha). Formato es-ES para números y euros.
...
- Tras responder, ofrece 1–3 acciones de seguimiento con la tool \`suggest_actions\` (solo lectura) en un paso aparte: \`openInternalSource\` hacia una superficie que hayas citado (\`holding\` con un id \`wl_hld_…\` ya leído, \`section\` como patrimonio/historico/objetivos, o \`figure\` como net_worth) y/o \`runSuggestedAnalysis\` con una pregunta útil de seguimiento. No pases URLs; la app descarta lo que no resuelva y pinta el resto como botones: nunca imprimas acciones en el texto, ni en JSON ni como lista de «Acciones recomendadas».${onboardingBlockText}${contextBlock}`;
}
