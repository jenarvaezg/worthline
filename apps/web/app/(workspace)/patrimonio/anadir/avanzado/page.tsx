...
export default function AnadirHoldingPage({
  ...
  return (
    <Suspense fallback={<FormRouteSkeleton label="Cargando añadir avanzado" />}>
      <AnadirAvanzadoContent {...(searchParams !== undefined ? { searchParams } : {})} />
    </Suspense>
  );
}
...
export async function AnadirAvanzadoContent({
  ...
  const { selectedScope, store, workspace } = await resolvePageShell({
    ...
    <>
      ...
      <section className="addHoldingPage" aria-label="Añadir holding">
        <div className="panelHeader addHoldingHeader">
          <h2>Añadir holding · modo avanzado</h2>
          <Link href="/patrimonio/anadir">← Volver al asistente</Link>
        </div>
        <p className="addHoldingLead">
          Modo avanzado: todos los instrumentos y campos técnicos en una sola pantalla. Si
          quieres lo mínimo, vuelve al asistente simple.
        </p>
        ...
        <form action={createHoldingAction} className="addHolding">
          <input type="hidden" name="returnTo" value="/patrimonio/anadir/avanzado" />
  ...
  return (
    <div className="addHoldingFields">
      ...
      {method === "derived" ? (
        <>
          <SymbolSearch
            basePath="/patrimonio/anadir/avanzado"
            instrument={id}
            pickedSymbol={
      ...
      {id === "mortgage" ? (
        <>
          ...
          <label className="addHoldingFull addHoldingInherit">
            ...
            />
            <input type="hidden" name={`inheritOwnership_${id}`} value="off" />
            <span>
