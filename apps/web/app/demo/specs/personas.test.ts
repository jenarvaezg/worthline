...
describe("seedPersona — joven (starter saver)", () => {
  ...
  it(
    "still shows the onboarding checklist (FIRE step pending)",
    async () => {
      const store = await createInMemoryStore();
      await seedPersona(store, JOVEN_SPEC, AS_OF);
...
describe("seedPersona — inversor (markets-heavy)", () => {
  it(
    "populates the market and term-locked rungs with a rich history",
    async () => {
      const store = await createInMemoryStore();
  ...
  it(
    "mirrors a frozen connected source with positions",
    async () => {
      const store = await createInMemoryStore();
      await seedPersona(store, INVERSOR_SPEC, AS_OF);
  ...
  it(
    "backfills Binance value before today's open period",
    async () => {
      const store = await createInMemoryStore();
      await seedPersona(store, INVERSOR_SPEC, AS_OF);
      ...
      expect(
        Math.abs((today?.valueMinor ?? 0) - (mayClose?.valueMinor ?? 0)),
      ).toBeLessThan(5_000_00);
  ...
  it(
    "computes strong FIRE progress for the configured scope",
    async () => {
      const store = await createInMemoryStore();
      await seedPersona(store, INVERSOR_SPEC, AS_OF);
