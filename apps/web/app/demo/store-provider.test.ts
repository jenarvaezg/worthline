...
const DEMO_STORE_RESEED_TIMEOUT_MS = 20_000;

describe("demo store provider", () => {
  it(
    "seeds and opens a usable familia store in memory",
    async () => {
    ...
    DEMO_STORE_TEST_TIMEOUT_MS,
  );

  it(
    "gives each call an independent in-memory database (nothing persists)",
    async () => {
...
describe("demo store cache (per process, #616)", () => {
  it(
    "reuses the seeded workspace for the same persona + as-of date",
    async () => {
  ...
  it(
    "reseeds for a different as-of date or persona",
    async () => {
