...
async function setupStore() {
  const store = await createFileBackedStore("worthline-patrimonio-");
...
describe("batchApplyValueUpdates — value-update-pass persistence", () => {
  ...
  test("empty batch is a no-op", async () => {
    ...
    expect(
      (await store.assets.readAssets()).find((a) => a.id === "a_only")!.currentValue
        .amountMinor,
    ).toBe(5_000);
    store.close();
  });
...
describe("updateLiability — full liability edit", () => {
  test("updates name, type, and associated asset", async () => {
    const store = await setupStore();
    await store.assets.createManualAsset({
      id: "a_house",
      name: "Casa",
    ...
    await store.liabilities.updateLiability("l_hip", {
      name: "Hipoteca nueva",
      type: "debt",
  ...
  test("updates ownership split for a liability", async () => {
    const store = await setupStore();
    await store.liabilities.createLiability({
      id: "l_own",
    ...
    await store.liabilities.updateLiability("l_own", {
      ownership: [
...
describe("updateAsset — full asset edit", () => {
  test("updates name, type, tier, and isPrimaryResidence", async () => {
    const store = await setupStore();
    await store.assets.createManualAsset({
      id: "a_edit",
      name: "Old Name",
    ...
    await store.assets.updateAsset("a_edit", {
      name: "New Name",
      type: "real_estate",
      liquidityTier: "housing",
      isPrimaryResidence: true,
    ...
    const asset = (await store.assets.readAssets()).find((a) => a.id === "a_edit")!;
  ...
  test("updates ownership split for an asset", async () => {
    const store = await setupStore();
    await store.assets.createManualAsset({
      id: "a_own",
      name: "Shared",
    ...
    await store.assets.updateAsset("a_own", {
      ownership: [
        { memberId: "m_ana", shareBps: 5_000 },
        { memberId: "m_jose", shareBps: 5_000 },
      ],
    ...
    const asset = (await store.assets.readAssets()).find((a) => a.id === "a_own")!;
...
describe("batchApplyAllValueUpdates — atomic asset+liability pass", () => {
  test("applies asset and liability updates in a single transaction", async () => {
    const store = await setupStore();
    await store.assets.createManualAsset({
      id: "a_cash",
      name: "Caja",
    ...
    await store.operations.batchApplyAllValueUpdates(
      [{ id: "a_cash", newValueMinor: 20_000 }],
      [{ id: "l_debt", newValueMinor: 3_000 }],
    );

    const asset = (await store.assets.readAssets()).find((a) => a.id === "a_cash")!;
    const liability = (await store.liabilities.readLiabilities()).find(
      (l) => l.id === "l_debt",
    )!;
  ...
  test("writes NOTHING when any amount is not an integer (atomicity guard)", async () => {
    const store = await setupStore();
    await store.assets.createManualAsset({
      id: "a_cash",
      name: "Caja",
...
describe("softDeleteLiability / restoreLiability — returns affected row count", () => {
  test("returns 1 when the liability exists", async () => {
    const store = await setupStore();
    await store.liabilities.createLiability({
      id: "l_del",
      name: "Para borrar",
    ...
    expect(
      await store.liabilities.softDeleteLiability("l_del", new Date().toISOString()),
    ).toBe(1);
    expect(await store.liabilities.restoreLiability("l_del")).toBe(1);
    store.close();
  });
  ...
  test("returns 0 when the id does not exist", async () => {
    const store = await setupStore();
    expect(
      await store.liabilities.softDeleteLiability("ghost_id", new Date().toISOString()),
    ).toBe(0);
    expect(await store.liabilities.restoreLiability("ghost_id")).toBe(0);
    store.close();
  });
});
