 ...
 *
 * Builds on `visibilityScopedTest` instead of "./fixtures" on purpose: this
 * journey provokes CSP violations, and a blocked request logs a console error,
 * which the shared fixture (rightly) treats as a failure. That fixture is what
 * makes every OTHER journey a `connect-src` regression guard for free — here we
 * need the opposite. What the fixture would also have caught is kept below:
 * uncaught page errors are collected and asserted empty, so opting out of the
 * console channel does not silently opt out of the rest of the net. The visibility
 * scope (#1351) is NOT part of that opt-out, which is why the base comes from
 * `./visible-page` rather than straight from Playwright.
 *
...
import { visibilityScopedTest as test } from "./visible-page";
