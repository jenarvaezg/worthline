...
async function liveCoinValue(store: WorthlineStore, assetId: string): Promise<number> {
  return (await store.assets.readAssets()).find((a) => a.id === assetId)!.currentValue
    .amountMinor;
}

describe("coin purchase-date ripple into snapshot history", () => {
  test("a coin ripples existing snapshots from its purchase date forward, not before", async () => {
    const store = await createInMemoryStore();
    await seed(store);
    // Two past snapshots straddling the coin's purchase date.
    await recordBuy(store, "2024-01-10", "10", "100"); // fund 1000.00
    await recordBuy(store, "2024-03-01", "5", "200"); // fund cost basis 10×100 + 5×200 = 2000.00
  ...
  test("each new trade ripples only from its own purchase date", async () => {
    const store = await createInMemoryStore();
    await seed(store);
    // A snapshot between the two coins' dates, and one after both.
    await recordBuy(store, "2024-02-15", "10", "100"); // fund 1000.00
    await recordBuy(store, "2024-03-15", "5", "100"); // fund cost basis 1500.00

    const { sourceId } = await connectNumista(store);
    await syncCoins(store, sourceId, [
    ...
    expect(await grossAt(store, "2024-02-15")).toBe(1_000_00 + 200_00);
    expect(await grossAt(store, "2024-03-15")).toBe(1_500_00 + 200_00 + 50_00);
    store.close();
  });

  test("a later price move never rewrites a rippled past snapshot (frozen)", async () => {
    const store = await createInMemoryStore();
    await seed(store);
    ...
    expect(await liveCoinValue(store, assetId)).toBe(500_00);
    expect(await grossAt(store, "2024-03-01")).toBe(1_000_00 + 100_00);
    store.close();
  });

  test("a sold coin leaves the live holding but stays in past snapshots", async () => {
    const store = await createInMemoryStore();
    await seed(store);
    await recordBuy(store, "2024-03-01", "10", "100"); // fund 1000.00
    ...
    expect(await liveCoinValue(store, assetId)).toBe(300_00);
    expect(await grossAt(store, "2024-03-01")).toBe(1_000_00 + 300_00);
    ...
    expect(await liveCoinValue(store, assetId)).toBe(100_00);
    expect(await grossAt(store, "2024-03-01")).toBe(1_000_00 + 300_00);
    store.close();
  });

  test("a coin with no purchase date is not rippled into history", async () => {
    const store = await createInMemoryStore();
    await seed(store);
    await recordBuy(store, "2024-03-01", "10", "100"); // fund 1000.00
...
describe("coin collection in freshly generated past snapshots", () => {
  test("a snapshot generated before the purchase date excludes the collection; after it includes it", async () => {
    const store = await createInMemoryStore();
    await seed(store);
    const { sourceId } = await connectNumista(store);
    ...
    await syncCoins(store, sourceId, [coin("c1", "2024-06-01", 300_00)]);

    // Backdated fund ops GENERATE fresh snapshots straddling the coin's date.
    await recordBuy(store, "2020-01-01", "10", "100"); // before the coin
    await recordBuy(store, "2024-12-01", "5", "100"); // after the coin

    // 2020 predates the coin → the collection is valued at 0 (no row), NOT its
    // full current value (the bug this fixes).
    expect(await grossAt(store, "2020-01-01")).toBe(10 * 100_00);
    // 2024-12 is after the purchase date → the coin is included, frozen at gen time.
    expect(await grossAt(store, "2024-12-01")).toBe(15 * 100_00 + 300_00);
    store.close();
  });

  test("fresh generation agrees with the #167 ripple on a shared date", async () => {
    ...
    const viaGeneration = await createInMemoryStore();
    await seed(viaGeneration);
    {
      const { sourceId } = await connectNumista(viaGeneration);
      await syncCoins(viaGeneration, sourceId, [coin("c1", "2024-06-01", 300_00)]);
      await recordBuy(viaGeneration, "2024-12-01", "5", "100"); // generates the snapshot
    }

    const viaRipple = await createInMemoryStore();
    await seed(viaRipple);
    {
      await recordBuy(viaRipple, "2024-12-01", "5", "100"); // snapshot exists first
      const { sourceId } = await connectNumista(viaRipple);
      await syncCoins(viaRipple, sourceId, [coin("c1", "2024-06-01", 300_00)]); // ripples it
    }

    expect(await grossAt(viaGeneration, "2024-12-01")).toBe(
      await grossAt(viaRipple, "2024-12-01"),
    );
    expect(await grossAt(viaGeneration, "2024-12-01")).toBe(5 * 100_00 + 300_00);
    viaGeneration.close();
    viaRipple.close();
  });
});
