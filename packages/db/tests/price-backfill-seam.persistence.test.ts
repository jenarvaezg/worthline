...
function rowsFor(store: WorthlineStore, holdingId: string) {
  return store.snapshots
    .readSnapshotHoldings({ holdingId, kind: "asset" })
    .sort((a, b) => a.dateKey.localeCompare(b.dateKey));
}

function rowAt(store: WorthlineStore, holdingId: string, dateKey: string) {
  return rowsFor(store, holdingId).find((r) => r.dateKey === dateKey);
}

describe("backfillInvestmentPricesAndRipple (#380)", () => {
  test("re-values the backfilled asset at units × price each month (no cost→price jump)", () => {
    ...
    const before = rowAt(store, "btc", "2026-01-10");
    ...
    const result = store.backfillInvestmentPricesAndRipple({
      assetId: "btc",
    ...
    const janVal = rowAt(store, "btc", "2026-01-10")?.valueMinor ?? 0;
  ...
  test("preserves OTHER frozen rows verbatim (cash row untouched)", () => {
    const store = createInMemoryStore();
    seed(store);
    ...
    const cashBefore = rowsFor(store, "cash").map((r) => ({
      dateKey: r.dateKey,
    ...
    const cashAfter = rowsFor(store, "cash").map((r) => ({
      dateKey: r.dateKey,
      valueMinor: r.valueMinor,
    }));
  ...
  test("keeps the reconciliation invariant (asset rows sum to gross_assets)", () => {
    const store = createInMemoryStore();
    seed(store);
    ...
    for (const snapshot of store.snapshots.readSnapshots()) {
      const rows = store.snapshots
        .readSnapshotHoldings({ scopeId: snapshot.scopeId })
  ...
  test("creates a missing monthly snapshot where a position existed but none was captured", () => {
    const store = createInMemoryStore();
    seed(store);
    ...
    expect(rowAt(store, "btc", "2026-02-01")).toBeUndefined();
    ...
    expect(rowAt(store, "btc", "2026-02-01")?.valueMinor).toBe(0.5 * 40000 * 100);
    store.close();
  });

  test("a month with no price is a gap, not a fabricated point", () => {
    const store = createInMemoryStore();
    seed(store);
  ...
  test("removes the cost→price cliff: every intermediate month carries a frozen price after backfill, and month-to-month deltas come ONLY from price moves (#380 AC contrast)", () => {
    const store = createInMemoryStore();
    seed(store);
    ...
    expect(rowAt(store, "btc", "2026-01-10")?.unitPrice).toBeUndefined();
  ...
  test("a sell to exactly 0 units fabricates no priced snapshot for sold-out months", () => {
    const store = createInMemoryStore();
    seed(store);
    ...
    const marBefore = rowAt(store, "btc", "2026-03-01");
    ...
    expect(rowAt(store, "btc", "2026-02-01")?.unitPrice).toBe("40000");
    ...
    const marAfter = rowAt(store, "btc", "2026-03-01");
    expect(marAfter).toEqual(marBefore);
    if (marAfter) expect(marAfter.unitPrice).toBeUndefined();
    store.close();
  });

  test("an empty pricesByDate is a no-op: zero created/updated, every row byte-identical, gaps list the held months", () => {
    const store = createInMemoryStore();
    seed(store);
    ...
    const btcBefore = rowsFor(store, "btc");
    const cashBefore = rowsFor(store, "cash");
    ...
    expect(result.gaps).toContain("2026-03-01");
    // No row was touched.
    expect(rowsFor(store, "btc")).toEqual(btcBefore);
    expect(rowsFor(store, "cash")).toEqual(cashBefore);
    store.close();
  });

  test("an EXISTING cost-basis snapshot on a gap month stays at cost (unit price undefined), and the month is a gap", () => {
    const store = createInMemoryStore();
    seed(store);
  ...
  test("stores exact decimal precision for a realistic non-round position", () => {
    const store = createInMemoryStore();
    seed(store);
  ...
  test("dry run returns the same per-scope counts the apply writes, in HOUSEHOLD mode (preview === confirm)", () => {
    ...
    const dry = createInMemoryStore();
    dry.workspace.initializeWorkspace({
      members: [
        { id: "mJ", name: "Jose" },
        { id: "mA", name: "Ana" },
      ],
    ...
    dry.assets.createInvestmentAsset({
      currency: "EUR",
      id: "btc",
      liquidityTier: "market",
      name: "Bitcoin",
    ...
    const preview = dry.backfillInvestmentPricesAndRipple({
      assetId: "btc",
      dryRun: true,
      pricesByDate: prices,
      source: "coingecko",
    ...
    const confirm = dry.backfillInvestmentPricesAndRipple({
      assetId: "btc",
      pricesByDate: prices,
      source: "coingecko",
      today: TODAY,
