...
/**
 * The floor of a saldo belongs to the CURVE, not to the rounding (#1693).
 *
 * This module used to carry its own copy of `toMinorInt` — the seam's round-to-the-
 * céntimo — with one extra line the other three copies did not have: every negative
 * was flattened to 0 before rounding. The copies were unified on the canonical
 * (unclamped) rounding, so these tests pin what the clamp was supposedly protecting:
 * an over-repayment reads 0 € because `applyCycleLumps` caps each lump at the
 * balance it is paid off and `computeBoundaries` floors the payoff month in Big —
 * the facts live there, on the month they happen, not in a blanket applied to every
 * figure that crosses the module.
 */
describe("el suelo del saldo lo pone la curva, no el redondeo (#1693)", () => {
  const LOAN: AmortizationPlanInput = {
    annualInterestRate: "0.03",
    initialCapitalMinor: 100_000_00,
    disbursementDate: "2020-01-01",
  ...
  test("una amortización mayor que el capital dentro del stub lee 0, no un negativo", () => {
    const stubPlan: AmortizationPlanInput = { ...LOAN, firstPaymentDate: "2020-03-01" };
    const paidOffInStub = amortizableBalanceAtDate({
      earlyRepayments: [
        { amountMinor: 150_000_00, mode: "reduce-term", repaymentDate: "2020-02-01" },
      ],
      plan: stubPlan,
      targetDate: "2020-02-15",
    });
    expect(paidOffInStub).toBe(0);
  });

  test("una amortización mayor que el saldo a mitad de ciclo lee 0 con las dos cadencias", () => {
    for (const cadence of ["step", "interpolated"] as const) {
      expect(
        amortizableBalanceAtDate({
          cadence,
          earlyRepayments: [
            { amountMinor: 500_000_00, mode: "reduce-term", repaymentDate: "2022-01-15" },
          ],
          plan: LOAN,
          targetDate: "2022-01-20",
        }),
      ).toBe(0);
      // Y sigue leyendo 0 en los meses siguientes, sin arrastrar signo.
      expect(
        amortizableBalanceAtDate({
          cadence,
          earlyRepayments: [
  ...
  test("una subida fuerte de tipo no deja un principal negativo que el clamp tapase", () => {
    // La cuota se recalcula sobre el saldo vivo y el plazo restante en cada
    // revisión, así que el principal de una cuota es siempre > 0: el clamp no
    // estaba protegiendo ninguna fila del cuadro, la estaba pudiendo falsear.
    const trace = amortizationScheduleTrace({
      plan: LOAN,
      revisions: [{ newAnnualInterestRate: "0.15", revisionDate: "2022-01-01" }],
      targetDate: "2029-12-01",
    });
    for (const period of trace.periods) {
      expect(period.principalMinor).toBeGreaterThan(0);
      expect(period.interestMinor).toBeGreaterThan(0);
      expect(period.closingBalanceMinor).toBeGreaterThanOrEqual(0);
    }
  });
});
