...
- A **holding** DERIVES its **valuation method** — an asset's from its **instrument**, a liability's from its **debt model** (ADR 0014, ADR 0101); it never carries it as an independent fact anyone can set. **Investment** is just the **derived** method (value = units × unit price, never set by hand), not a kind of its own.
